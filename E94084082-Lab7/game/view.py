...
class GameView:
    ...
    def draw_money(self, money: int):
        ...
        # 利用 render 來寫字，參數由左至右為:"要寫上的字 , 是否開啟抗鋸齒(True的話字體較平滑) , 字體顏色"
        text = self.font.render(f"Money: {money}", True, (255, 255, 255)) 
        self.win.blit(text, (5, 45))     # 將 money 的資訊 blit 到遊戲畫面上 ，(5, 45)為資訊位置的左上角座標

    def draw_wave(self, wave: int):
        """(Q2.2)render the wave"""
        # 利用 render 來寫字，參數由左至右為:"要寫上的字 , 是否開啟抗鋸齒(True的話字體較平滑) , 字體顏色"
        text = self.font.render(f"Wave: {wave}", True, (255, 255, 255))
        self.win.blit(text, (5, 15))     # 將 wave 的資訊 blit 到遊戲畫面上，(5, 15)為資訊位置的左上角座標
